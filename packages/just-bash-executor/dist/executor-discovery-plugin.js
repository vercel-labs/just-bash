 ...
 */
import { definePlugin, Source, ToolInvocationResult, ToolRegistration, } from "@executor-js/sdk";
/**
        ...
        init: async (ctx) => {
            ...
            const addSource = async (def) => {
                if (def.kind === "custom" && def.tools) {
                    await addCustomSource(ctx, def, registeredToolIds, registeredSourceIds);
                }
                else {
                    throw new Error(`Unsupported source kind: "${def.kind}" for discovery plugin. ` +
...
async function addCustomSource(ctx, def, registeredToolIds, registeredSourceIds) {
    const tools = def.tools;
    if (!tools)
        return;
    // Register the source
    await ctx.sources.registerRuntime(new Source({
        id: def.name,
        name: def.name,
        kind: "custom",
        runtime: true,
        ...
        const handler = {
            invoke: async (args) => {
                try {
                    ...
                    return new ToolInvocationResult({
                        data: null,
                        // @banned-pattern-ignore: tool-author-supplied error text from
                        // an inline tool's own throw; not host/internal data.
